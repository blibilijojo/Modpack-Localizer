...
from gui.workbench_find_replace_mixin import WorkbenchFindReplaceMixin
from gui.workbench_ai_mixin import WorkbenchAIMixin

class TranslationWorkbench(WorkbenchAIMixin, WorkbenchFindReplaceMixin, ttk.Frame):
        ...
        def jump_to_history(event):
            ...
            if selected_item:
                ...
                if 0 <= index < len(self.operation_history):
                    ...
                    target_index = index if is_redo_operation else max(0, index - 1)
                    target_state = self.operation_history[target_index]['state']
                    if len(target_state) == 1 and self.operation_history[target_index]['type'] in ('EDIT', 'REPLACE'):
                        ns = list(target_state.keys())[0]
                        self.translation_data[ns] = copy.deepcopy(target_state[ns])
                    else:
                        self.translation_data = copy.deepcopy(target_state)
