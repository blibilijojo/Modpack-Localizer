...
- **自动同步上游仓库代码**：系统会在上传前自动同步原仓库，确保代码与最新版本保持一致，这是系统运行的必要操作
- 自动创建 Pull Request
- 分支管理和文件上传
...
```
Modpack-Localizer-2.7.0/
├── .github/          # GitHub 相关配置
│   └── workflows/    # CI/CD 工作流配置
│       └── build-release.yml # 构建发布工作流
├── core/             # 核心功能模块
│   ├── __init__.py           # 模块初始化
│   ├── builder.py            # 资源包构建器
...
│   └── workflow.py           # 工作流管理
├── examples/         # 示例代码
│   └── multithreading_example.py # 多线程示例
├── gui/              # 图形用户界面
│   ├── dialog/            # 对话框
│   │   ├── __init__.py            # 模块初始化
│   │   └── github_version_select.py # GitHub版本选择对话框
│   ├── settings_components/  # 设置面板组件
│   │   ├── advanced_settings.py      # 高级设置
│   │   ├── ai_settings.py            # AI 设置
│   │   ├── basic_settings.py         # 基础设置
│   │   ├── curseforge_settings.py    # CurseForge设置
│   │   ├── external_services_settings.py # 外部服务设置
│   │   ├── general_settings.py       # 通用设置
...
│   ├── github_download_ui.py     # GitHub下载界面
