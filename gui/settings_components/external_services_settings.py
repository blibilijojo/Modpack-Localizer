...
import threading
from gui.settings_components.shared_utils import test_github_authentication
...
class ExternalServicesSettings:
    ...
    def _test_github_auth(self):
        ...
        def test_task():
            try:
                success, message = test_github_authentication(repo, token)
                self.github_status_var.set(message)
            finally:
