...
def _get_builtin_curseforge_key() -> str | None:
    try:
        from utils.builtin_secrets import get_builtin_curseforge_key
        return get_builtin_curseforge_key()
    except ImportError:
        return None
...
def _handle_builtin_key(config: dict) -> dict:
    builtin_key = _get_builtin_curseforge_key()

    if builtin_key:
        current_key = config.get('curseforge_api_key', '').strip()
...
def load_config() -> dict:
    ...
    try:
        current_mtime = CONFIG_FILE_PATH.stat().st_mtime if CONFIG_FILE_PATH.exists() else 0.0
    except OSError:
        current_mtime = 0.0
    ...
    try:
        ...
        config = _handle_builtin_key(config)

        config, config_updated = _migrate_config(config)
...
def save_config(config_data: dict):
    ...
    try:
        ...
        try:
            _config_cache_mtime = CONFIG_FILE_PATH.stat().st_mtime
        except OSError:
...
def update_config_batch(updates: dict) -> dict:
    config = load_config()
